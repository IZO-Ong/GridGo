...
export default function AuthPage() {
  ...
  const handleAction = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        localStorage.setItem("gridgo_token", data.token);
        localStorage.setItem("gridgo_user", data.username);
        router.push("/");
        router.refresh();
      } else {
        setIsVerifying(true);
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/verify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: formData.username, code: otp }),
      });
  ...
  return (
    <div className="flex flex-col items-center justify-center h-[calc(100vh-250px)] min-h-[600px] space-y-4">
      {error && (
        <div
      ...
      <div className="w-full max-w-[380px] border-4 border-black p-6 bg-white shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
        ...
        {!isVerifying ? (
          <>
            ...
            <button
              onClick={handleOAuth}
              className="w-full border-2 border-black py-2 font-black uppercase text-[11px] flex items-center justify-center gap-3 hover:bg-zinc-100 cursor-pointer transition-colors active:translate-y-0.5"
            >
